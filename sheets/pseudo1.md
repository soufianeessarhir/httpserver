...
### handleClientData()
```
FUNCTION handleClientData(client_fd):
    ...
    ELSE IF read error:
        IF error is EAGAIN or EWOULDBLOCK:
            RETURN (no more data available now)
        ELSE:
...
### handleClientWrite()
```
FUNCTION handleClientWrite(client_fd):
    GET connection from client_connections map
...
FUNCTION run():
    ...
    WHILE true:
        ...
        FOR EACH event:
            ...
            ELSE IF event is EPOLLIN:
                HANDLE client data
            ELSE IF event is EPOLLOUT:
                HANDLE client write
